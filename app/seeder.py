...
def apply_vertical_kit(tenant_id: str, vertical: str):
    ...
    try:
        ...
        for item in templates:
            cursor.execute(
                "INSERT INTO entities (tenant_id, type, data_json) VALUES (?, ?, ?)",
                (tenant_id, item["type"], json.dumps(item["data"]))
            )
        conn.commit()
        logger.info(f"Successfully applied vertical kit '{vertical}' for tenant '{tenant_id}'.")
        return True
