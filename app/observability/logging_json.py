...
class JSONFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        ...
        if hasattr(record, "tenant_id") and record.tenant_id != "-":
            log_data["tenant_hash"] = hashlib.sha256(str(record.tenant_id).encode()).hexdigest()[:12]
        if record.exc_info:
...
def init_json_logging(app: Flask) -> None:
    ...
    if logger.handlers:
        for h in logger.handlers:
            if isinstance(h, RotatingFileHandler) and h.baseFilename == str(log_file.absolute()):
    ...
    def handle_exception(e):
        ...
        response = jsonify({"error": "Internal Server Error", "request_id": getattr(g, "request_id", "-")})
