    ...
    *,
    ...
    if ext == ".pdf":
        if fitz is None:
            ...
            return {"ok": False, "error_code": "pymupdf_missing"}
        ...
        try:
            ...
            for i in range(min(len(doc), OCR_MAX_PAGES)):
                ...
                if len(p_text) > MIN_TEXT_LEN_BEFORE_OCR:
                    full_text.append(p_text)
                else:
                    # 2. Render to image and OCR
            ...
            text_out = "\n---\n".join(full_text)
            error_code = None
            truncated = 1 if len(doc) > OCR_MAX_PAGES else 0
        except Exception as e:
            text_out = None
            error_code = f"pdf_processing_failed:{str(e)[:50]}"
            truncated = 0
...
def process_dirty_note(image_path: str, tenant_id: str = "SYSTEM_OCR"):
    """
    Liest ein Bild oder PDF lokal aus und extrahiert Daten via 'Meister'-LLM.
    ...
    try:
        if "demo" in image_path.lower():
            ...
            demo_path = "docs/demo/FAKE_OCR_RESULT.txt"
            if os.path.exists(demo_path):
                with open(demo_path, "r") as f:
                    text = f.read()
            else:
                text = "Demo Scan: Baustoff Union GmbH, 23.02.2026, 428.50 EUR. Zement CEM II (12), Bausand (4)."
        elif image_path.lower().endswith(".pdf"):
            logger.info(f"PDF-MODUS: Verarbeite Dokument: {image_path}")
            doc = fitz.open(image_path)
            full_text = []
            for page in doc:
                # 1. Versuche direkten Text-Extrakt (falls kein Scan)
                page_text = page.get_text().strip()
                if len(page_text) > 50:
                    full_text.append(page_text)
                else:
                    # 2. Falls kein Text, rendere Seite und nutze Tesseract
        ...
        prompt = (
            ...
            '{"vendor": "Name", "date": "YYYY-MM-DD", "total": 0.0, "currency": "EUR", "items": [{"name": "item", "qty": 1}]}'
            f"\n\nSCAN-TEXT:\n{text}"
        )

        json_str = ask_local_ai(prompt, tenant_id=tenant_id)

        # 3. Auto-Booking (Agenten-Schlag)
        try:
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                entity_id = f"ocr_{uuid.uuid4().hex[:8]}"
                conn.execute(
                    "INSERT INTO entities (id, tenant_id, type, data_json) VALUES (?, ?, ?, ?)",
                    (entity_id, tenant_id, "ocr_receipt", clean_json)
                )
                conn.commit()
                conn.close()
            ...
            logger.error(f"JSON Parsing/Booking failed: {ej}")

        return {"text": json_str, "data": None}
