...
class CRDTContactManager:
    ...
    def update_customer_name(self, kdnr: str, name: str):
        ...
        with self._db() as con:
            ...
            row = con.execute("SELECT crdt_meta, customer_name, phone FROM docs_index WHERE kdnr=?", (kdnr,)).fetchone()
            ...
            if not meta:
                meta = {
                    "customer_name": LWWRegister(row["customer_name"], timestamp=0.0).to_dict(),
    ...
    def merge_from_remote(self, kdnr: str, remote_meta_json: str, remote_customer_name: str, remote_phone: str = None):
        """
        ...
        with self._db() as con:
            ...
            if "customer_name" not in remote_meta and remote_customer_name:
                remote_meta["customer_name"] = LWWRegister(remote_customer_name, timestamp=0.0).to_dict()
            if "phone" not in remote_meta and remote_phone:
                remote_meta["phone"] = LWWRegister(remote_phone, timestamp=0.0).to_dict()
