...
def _ensure_column(
    ...
    if not _column_exists(con, table, column):
        sql = "ALTER TABLE %s ADD COLUMN %s %s" % (table, column, col_type)
        con.execute(sql)
...
def db_init() -> None:
    ...
    with _DB_LOCK:
        ...
        try:
            ...
            # Phase 1.6: CRDT Columns for Global Mesh Sync
            for table in ["docs", "versions", "entities", "tasks"]:
                _ensure_column(con, table, "lamport_ts", "INTEGER NOT NULL DEFAULT 0")
                _ensure_column(con, table, "node_id", "TEXT")
                _ensure_column(con, table, "is_deleted", "INTEGER NOT NULL DEFAULT 0")
...
def _analyze_worker(token: str) -> None:
    try:
        ...
        # Phase 5: LLM Refined Extraction (Refinement)
        try:
            from app.ai.extraction_agent import extraction_agent
            llm_data = extraction_agent.refine_extraction(text)
            if llm_data:
                # Bevorzugte LLM-Werte wenn vorhanden und sinnvoll
                if llm_data.get("doctype"):
                    d["doctype_suggested"] = llm_data["doctype"]
                if llm_data.get("doc_date"):
                    # Wir prüfen ob es ein valides Datum YYYY-MM-DD ist
                    import re
                    if re.match(r"\d{4}-\d{2}-\d{2}", llm_data["doc_date"]):
                        d["doc_date_suggested"] = llm_data["doc_date"]
                if llm_data.get("vendor_name"):
                    d["vendor_name"] = llm_data["vendor_name"]
                if llm_data.get("total_amount"):
                    d["total_amount"] = llm_data["total_amount"]
                if llm_data.get("invoice_no"):
                    d["invoice_no"] = llm_data["invoice_no"]
                if "vat_valid" in llm_data:
                    d["vat_valid"] = llm_data["vat_valid"]
                if llm_data.get("net_amount"):
                    d["net_amount"] = llm_data["net_amount"]
                if llm_data.get("vat_amount"):
                    d["vat_amount"] = llm_data["vat_amount"]
        except Exception as e:
            logger.warning(f"LLM Refinement fehlgeschlagen: {e}")
