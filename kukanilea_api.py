...
def root():
    return jsonify(
        ...
        auth=(
            "X-API-Key required (except /health,/ui)" if API_KEY else "open (dev mode)"
        ),
    )
...
def upload():
    ...
    return jsonify(
        ok=True, token=token, path=str(dest), filename=filename, doc_id=doc_id
    )
...
def process(token: str):
    ...
    done_payload = {
        ...
        or answers.get("mandant")
        or p.get("tenant_suggested", ""),
