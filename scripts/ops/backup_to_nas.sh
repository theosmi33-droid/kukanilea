...
if [[ "$DRY_RUN" -eq 1 ]]; then
  {
    ...
    echo "target_path=none"
    echo "checksum_sha256=dry-run"
    echo "backup_size_bytes=0"
    echo "compression_ratio=0.0000"
    echo "rto_seconds=0"
    echo "rpo_seconds=0"
  } > "$REPORT_FILE"
...
SOURCE_SIZE_BYTES="$(tar -C instance -cf - . | wc -c | tr -d ' ')"
COMPRESSION_RATIO="$(awk -v s="$SOURCE_SIZE_BYTES" -v b="$BACKUP_SIZE_BYTES" 'BEGIN { if (s > 0) printf "%.4f", b/s; else printf "0.0000" }')"
