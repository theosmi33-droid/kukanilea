...
async def stress_worker(client, stats):
    ...
    for _ in range(TOTAL_REQUESTS // CONCURRENCY):
        for target in targets:
            ...
            try:
                headers = {
                    "X-Tenant-ID": "stress-tenant",
                    "X-User-ID": "bot",
                    "X-Role": "ADMIN",
                }
                if target["method"] == "GET":
                    resp = await client.get(
                        target["url"], headers=headers, timeout=10.0
                    )
                else:
                ...
                stats["status_codes"][resp.status_code] = (
                    stats["status_codes"].get(resp.status_code, 0) + 1
                )

            except Exception as e:
                stats["errors"] += 1
                stats["error_types"][type(e).__name__] = (
                    stats["error_types"].get(type(e).__name__, 0) + 1
                )
...
async def main():
    ...
    p99_latency = (
        sorted(stats["latencies"])[int(len(stats["latencies"]) * 0.99)]
        if stats["latencies"]
        else 0
    )
