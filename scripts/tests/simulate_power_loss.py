...
def simulate_crash():
    ...
    con.execute("CREATE TABLE IF NOT EXISTS test_data (id INTEGER PRIMARY KEY, data TEXT)")
    
    # Transaktion starten
    con.execute("BEGIN EXCLUSIVE")
    for i in range(10000):
        con.execute("INSERT INTO test_data (data) VALUES (?)", (f"Wichtige Kunden-Daten {i}",))
...
def verify_recovery():
    ...
    try:
        ...
        if result == "ok":
            print("[BAGGER-TEST] Integritaetspruefung bestanden: DB ist 'ok' und nicht korrupt.")
        else:
        ...
        print(f"[BAGGER-TEST] Gefundene Datensaetze: {count} (Erwartet: 0, da Transaktion abgebrochen wurde)")
        
        if count == 0:
            print("[PASSED] Bagger-Test erfolgreich! Die SQLite WAL-Architektur hat den Datenmuell blockiert.")
        else:
