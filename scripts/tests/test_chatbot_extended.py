...
def test_chatbot():
    ...
    with app.app_context():
        ...
        assert res["text"] == "Bitte gib eine Nachricht ein.", (
            f"Failed empty msg: {res}"
        )
        ...
        with patch("app.agents.llm_ollama.generate") as mock_llm:
            ...
            assert res.get("action") is not None, (
                "Failed to parse action from LLM output"
            )
        ...
        with patch("app.agents.llm_ollama.generate") as mock_llm:
            ...
            assert "Hier sind die Fakten" in res["text"] or "Fakten" in res["text"], (
