...
def run_step(name, command_args, cwd=PROJECT_ROOT):
    ...
    try:
        ...
        result = subprocess.run(
            command_args, check=True, capture_output=True, text=True, cwd=cwd
        )
...
def main():
    ...
    summary = {"timestamp": DATE_STR, "steps": {}}

    # 1. Unit & Integration Tests
    success, output = run_step(
        "Unit Tests", ["pytest", "tests/", "--junitxml=dist/evidence/unit_results.xml"]
    )
    ...
    overall_pass = all(
        s in ["PASS", "PASS (Evidence from triage_report.json)"]
