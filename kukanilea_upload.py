...
TENANT_ALLOWLIST = [
    x.strip().lower() for x in os.environ.get("TOPHANDWERK_TENANTS", "").split(",") if x.strip()
]
...
def index():
    ...
    return _render_base(
        render_template_string(HTML_INDEX, items=items, meta=meta, admin_tools=admin_tools),
        active_tab="upload",
    )
...
def upload():
    ...
    _audit(
        "upload", target=str(dest), meta={"token": token, "filename": filename, "tenant": tenant}
    )
...
def review_kdnr(token):
    ...
    if p.get("status") == "ANALYZING":
        return _render_review(
            token, _card_info("Analyse läuft noch… bitte kurz warten oder zurück zur Übersicht.")
        )
    ...
    if request.method == "POST":
        ...
        existing = (
            find_existing_customer_folders(BASE_PATH, val) if find_existing_customer_folders else []
        )
        ...
        _audit(
            "review_set_kdnr",
            target=token,
            meta={"tenant": tenant, "kdnr": w["kdnr"], "customer_status": w["customer_status"]},
        )
...
def review_confirm(token):
    ...
    if request.method == "POST" and request.form.get("final") == "1":
        ...
        if not src.exists():
            ...
            return _render_base(
                _card_error("Datei im Eingang nicht gefunden (evtl. verschoben/gelöscht).")
            )
        ...
        done_payload = {
            ...
            "customer_status": (
                w.get("customer_status") or ("BESTAND" if answers.get("use_existing") else "NEU")
            ),
        }
        ...
        _audit(
            ...
            meta={"tenant": tenant, "kdnr": answers["kdnr"], "doctype": done_payload["doctype"]},
        )
...
def assistant():
    ...
    if q:
        if assistant_search is None:
            return _render_base(
                _card_error("Assistant ist im Core noch nicht verfügbar."), active_tab="assistant"
            )
        try:
