...
LEGACY_FLOW_ACTION_ALIASES: dict[str, str] = {
    "email_extract_task": "mail.message.extract",
    "email_match_project": "mail.project.match",
    "calendar_check_conflict": "calendar.appointment.check_conflict",
    "calendar_propose_slot": "calendar.appointment.propose_slot",
    "document_extract": "dms.document.extract",
    "document_suggest_deadline_task": "tasks.task.suggest_deadline",
    "messenger_extract_followup": "messenger.message.extract_followup",
    "system_create_audit_entry": "system.audit.create_entry",
    "system_notify_admin": "system.admin.notify",
    "deadline_sync_calendar_task": "calendar.task.sync_deadline",
    "document_detect_customer_project": "dms.document.detect_customer_project",
    "document_propose_action": "dms.document.propose_action",
}
...
class AtomicActionRegistry:
    ...
    def register_legacy_alias(self, legacy_action_id: str, canonical_action_id: str) -> None:
        self._legacy_aliases[str(legacy_action_id)] = str(canonical_action_id)

    def resolve_action_id(self, action_id: str) -> str | None:
        if action_id in self._handlers:
            return action_id
        canonical = self._legacy_aliases.get(action_id)
        if canonical and canonical in self._handlers:
            warnings.warn(
                f"Flow action '{action_id}' is deprecated and will be removed; use '{canonical}'.",
                DeprecationWarning,
                stacklevel=2,
            )
            return canonical
...
def create_default_registry() -> AtomicActionRegistry:
    ...
    registry.register(
        "calendar.appointment.check_conflict",
        lambda p: {
    ...
    registry.register(
        "calendar.appointment.propose_slot",
        lambda p: {
    ...
    registry.register(
        "calendar.task.sync_deadline",
        lambda p: {"sync_ref": f"sync:{p.get('deadline_id', 'n/a')}"},
    )
    registry.register(
        "dms.document.detect_customer_project",
        lambda p: {
    ...
    registry.register(
        "dms.document.propose_action",
        lambda p: {"suggested_action": str(p.get("suggested_action") or "Aufgabe anlegen")},
    )

    for legacy_action, canonical_action in LEGACY_FLOW_ACTION_ALIASES.items():
...
def build_core_flows() -> dict[str, FlowDefinition]:
    return {
        "flow_email_to_task": FlowDefinition(
            ...
            steps=(
                FlowStep("extract_email", "mail.message.extract"),
                FlowStep("create_task", "tasks.task.suggest_deadline", writes_state=True, required_tool="tasks"),
            ),
        ...
        "flow_email_project_task": FlowDefinition(
            ...
            steps=(
                FlowStep("extract_email", "mail.message.extract"),
                FlowStep("match_project", "mail.project.match", required_tool="projects"),
                FlowStep("create_task", "tasks.task.suggest_deadline", writes_state=True, required_tool="tasks"),
            ),
        ...
        "flow_calendar_conflict_suggest": FlowDefinition(
            ...
            steps=(
                FlowStep("check_conflict", "calendar.appointment.check_conflict", required_tool="calendar"),
                FlowStep("propose_slot", "calendar.appointment.propose_slot"),
            ),
        ...
        "flow_document_extract_deadline_task": FlowDefinition(
            ...
            steps=(
                FlowStep("extract_document", "dms.document.extract", required_tool="ocr"),
                FlowStep("propose_deadline_task", "tasks.task.suggest_deadline", writes_state=True, required_tool="tasks"),
            ),
        ...
        "flow_messenger_followup_task": FlowDefinition(
            ...
            steps=(
                FlowStep("extract_followup", "messenger.message.extract_followup"),
                FlowStep("create_followup_task", "tasks.task.suggest_deadline", writes_state=True, required_tool="tasks"),
            ),
        ...
        "flow_license_event_audit_admin": FlowDefinition(
            ...
            steps=(
                FlowStep("create_audit", "system.audit.create_entry", writes_state=True, required_tool="audit"),
                FlowStep("notify_admin", "system.admin.notify", writes_state=True, required_tool="settings"),
            ),
        ...
        "flow_deadline_sync": FlowDefinition(
            ...
            steps=(
                FlowStep("sync_deadline", "calendar.task.sync_deadline", writes_state=True, required_tool="calendar"),
            ),
        ...
        "flow_document_intake_classify_action": FlowDefinition(
            ...
            steps=(
                FlowStep("detect_entities", "dms.document.detect_customer_project"),
                FlowStep("propose_action", "dms.document.propose_action", writes_state=True, required_tool="tasks"),
            ),
