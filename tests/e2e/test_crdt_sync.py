...
def run_sync_test():
    ...
    for db in [hub_db, peer_db]:
        with sqlite3.connect(db) as con:
            con.execute("INSERT INTO docs_index (kdnr, customer_name, phone) VALUES (?, ?, ?)", 
    ...
    with sqlite3.connect(peer_db) as con:
        ...
        row = con.execute("SELECT crdt_meta FROM docs_index WHERE kdnr=?", (kdnr,)).fetchone()
        ...
        con.execute("UPDATE docs_index SET phone=?, crdt_meta=? WHERE kdnr=?", 
                    ("0170-999", json.dumps(meta), kdnr))
        con.commit()

    print(f"--- Phase 2: Hub updated Name (T1), Peer updated Phone (T2) ---")

    # Phase 3 (Merge): Merge Peer data into Hub
    with sqlite3.connect(peer_db) as con:
        con.row_factory = sqlite3.Row
        peer_row = con.execute("SELECT crdt_meta, customer_name, phone FROM docs_index WHERE kdnr=?", (kdnr,)).fetchone()
        
    hub_mgr.merge_from_remote(kdnr, peer_row["crdt_meta"], peer_row["customer_name"], peer_row["phone"])
    
    with sqlite3.connect(hub_db) as con:
        con.row_factory = sqlite3.Row
        final_row = con.execute("SELECT * FROM docs_index WHERE kdnr=?", (kdnr,)).fetchone()
        
    print(f"Final State on Hub: Name='{final_row['customer_name']}', Phone='{final_row['phone']}'")
    
    success = (final_row['customer_name'] == "Hans Mueller (Stammkunde)" and 
               final_row['phone'] == "0170-999")
